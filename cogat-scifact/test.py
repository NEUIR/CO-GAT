...
def eval_model(model, valid_dataloader,outpath):
    ...
    with torch.no_grad():
        for step, batch in enumerate(valid_dataloader):
            ...
            assert len(predict_idxs) == len(ids) == len(dids)
            for step in range(len(predict_idxs)):
                ...
                data_dict[ids[step]][dids[step]]["label"] = label_list[predict_idxs[step]]
...
def main():
    ...
    valid_dataset = data_loader_test(args, tokenizer)
